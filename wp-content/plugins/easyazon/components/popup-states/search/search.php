...
class EasyAzon_Components_PopupStates_Search {
	...
	public static function output_state() {
		$locales = array_filter(easyazon_get_locales(), function($locale) {
			$associate_tags = trim(easyazon_get_setting(sprintf('associates_%s', strtolower($locale))));

			return false === empty($associate_tags);
	...
	public static function add_search_result_columns($columns) {
		$columns['image'] = __('Image', 'easyazon');
		$columns['title'] = __('Title', 'easyazon');
		$columns['insert'] = __('Insert', 'easyazon');
